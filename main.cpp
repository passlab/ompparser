...
//const char* input = "omp requires ext_user_test reverse_offload ext_user_test2 reverse_offload unified_address unified_shared_memory,atomic_default_mem_order(acq_rel) dynamic_allocators";
//const char* input = "omp target data if(target data:3456) device(5) use_device_ptr(*p,*pt,*ptr) use_device_addr(5)";
//const char* input = "omp target enter data if(target enter data:3456) device(5) depend(iterator(int bba=4:120:2), in:m, n) map(close,mapper(1234),always,to:q,w,e) nowait ";
