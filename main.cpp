...
int main( int argc, const char* argv[] ) {
	...
	const char* input = "omp metadirective when ( user = { condition (b < 14) } : ) when ( user = { condition (a < 4) } device = { kind (cpu) isa(amd_avx512f) } implementation = {vendor(amd)} : parallel private (a, bb)) when ( construct = {parallel (score(4) : private (e) )} : parallel private (a, bb)) default (parallel shared (c, dd))";

    //const char* input = "omp declare variant (...) match ( user = { condition (a < 4) } device = { kind (fpga) isa(avx512f) } implementation = {amd})";
