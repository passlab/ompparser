...
{
    const char* input = "omp parallel for private (a[foo(x, goo(x, y)):100], b[1:30], c) num_threads (3*5+4/(foo(x)+10)) firstprivate (foo(x), y) shared (a, b, c[1:10]) copyin (a[foo(goo(x)):20]) default(shared) default(none)";
